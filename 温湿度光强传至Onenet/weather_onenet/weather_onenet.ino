...
//配置ESP8266WIFI设置
#define SSID "liujice"    //填写2.4GHz的WIFI名称，不要使用校园网
#define PASSWORD "liujice123"//填写自己的WIFI密码
#define HOST_NAME "api.heclouds.com"  //API主机名称，连接到OneNET平台，无需修改
#define DEVICE_ID "858424058"       //填写自己的OneNet设备ID
#define HOST_PORT (80)                //API端口，连接到OneNET平台，无需修改
String APIKey = "ksmUyixdR1lBPwIsJAzsTtWjgFY="; //与设备绑定的APIKey

#define INTERVAL_SENSOR 5000 //定义传感器采样及发送时间间隔

//创建dht11示例
dht11 DHT11;
int threshold = 100; //光强值，根据实际情况调整   可以实现根据当时光强自动开关灯的效果
#define DHT11PIN 4  //定义DHT11接入Arduino的引脚
...
void loop() {
  ...
  if (millis() - net_time1 > INTERVAL_SENSOR) //发送数据时间间隔
  {
    ...
    float sensor_lig = analogRead(A3); //读取模拟口A3
    ...
    if (sensor_lig > threshold ) //晚上光线暗，n值变大
